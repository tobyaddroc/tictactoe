...
DWORD WINAPI __AHXRSOCKET__::__t__createServerRes(LPVOID lpParams) {
	...
	do {
		...
		if ( i_result > 0 ) {
			...
			if ( Socket_Response->fpResData != NULL )
				Socket_Response->fpResData( Socket_Response->SocketResponse, Socket_Response->addrRes, Socket_Response->sockAddr, c_rec );
		}
		else if ( i_result == 0 )
	...
	Socket_Response->fpDisconnected( Socket_Response->SocketResponse, Socket_Response->addrRes, Socket_Response->sockAddr );

...
void AHXRCLIENT::close() {
	...
	WSASendDisconnect( Socket_Client, wsa_buf );
	delete wsa_buf;
...
void AHXRCLIENT::listen(FP_RES fpRes, bool thread) {
	...
	if ( thread )
	{
		CreateThread( 0, 0, __AHXRSOCKET__::__t__listenThread, this, 0, 0 );
	}
	else
	{
		this->__listenfunc( fpRes );
	}
}
...
bool AHXRCLIENT::send_data( const char * sendChar, int len ) {
	bool		b_result;

	if (send(Socket_Client, sendChar, len, 0) == SOCKET_ERROR) {
		closesocket(Socket_Client);
		WSACleanup();
		b_result = false;
	}

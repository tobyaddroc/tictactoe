...
bool __AHXRSOCKET__::__createserver(AHXRSERVER * const & server_class, PCSTR port, int protocol, FPC_CALLBACK fpConnected, FPS_CALLBACK fpData, FPD_CALLBACK fpDisconnected) {
	WSADATA				wsaData;
	...
	while (1)  {
		...
		Socket_Client = accept(Socket_Listen, &cliparams, new int( sizeof cliparams ));
		...
		s_params->sockAddr = ( sockaddr_in * )&cliparams;
		fpDisconnected != NULL ? s_params->fpDisconnected = fpDisconnected : s_params->fpDisconnected = NULL;

		CreateThread(0, 0, __AHXRSOCKET__::__t__createServerRes, s_params, 0, 0);
		
		if( fpConnected != NULL )
			fpConnected(Socket_Client, res, ( sockaddr_in * ) & cliparams );
	}
...
void AHXRSERVER::send_data(SOCKET clientSocket, const char * sendChar, int len) {
	if (send(clientSocket, sendChar, len, 0) == SOCKET_ERROR) {
...
bool AHXRSERVER::start_server(PCSTR port, int protocol, FPC_CALLBACK fpConnected, FPS_CALLBACK fpData, FPD_CALLBACK fpDisconnected, bool thread) {
	if (!thread) return __AHXRSOCKET__::__createserver(this, port, protocol, fpConnected, fpData, fpDisconnected);
	...
	server_data->fpDisconnected		= fpDisconnected;
...
DWORD WINAPI __AHXRSOCKET__::__t__startserver(LPVOID lpParams) {
	...
	__AHXRSOCKET__::__createserver(s_data->s_class, s_data->port, s_data->protocol, s_data->fpConnected, s_data->fpData, s_data->fpDisconnected);
	return 0;
}
